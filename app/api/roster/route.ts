    ...
    const membersWithHours = await Promise.all(
      members.map(async (m) => {
        ...
        return {
          ...
          lastSeen: m.lastSeen,
          lastSeenDate: m.lastSeen ? new Date(m.lastSeen).toLocaleDateString("en-US", { timeZone: "UTC", year: "numeric", month: "2-digit", day: "2-digit" }) : null,
          lastSeenTime: m.lastSeen ? new Date(m.lastSeen).toLocaleTimeString("en-US", { timeZone: "UTC", hour: "2-digit", minute: "2-digit", hour12: false }) + "Z" : null,
